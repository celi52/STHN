...
class MLPMixer(nn.Module):
    ...
    def forward(self, edge_feats, edge_ts, batch_size, inds):
        ...
        # x = x.view(-1, self.per_graph_size, x.shape[1]) 
...
class EdgePredictor_per_node(torch.nn.Module):
    ...
    def __init__(self, dim_in_time, dim_in_node, predict_class):
        ...
        self.src_fc = torch.nn.Linear(dim_in_time, 100) # dim_in_time + dim_in_node
        self.dst_fc = torch.nn.Linear(dim_in_time, 100)
...
class Mixer_multiclass(nn.Module):
    def __init__(self, mlp_mixer_configs, edge_predictor_configs):
        super(Mixer_multiclass, self).__init__()
