...
def run(model, optimizer, args, subgraphs, df, node_feats, edge_feats, MLAUROC, MLAUPRC, mode):
    ...
    for ind in range(len(train_loader)):
        ...
        loss, pred, edge_label = model(inputs, has_temporal_neighbors, neg_samples, subgraph_node_feats)
...
def link_pred_train(model, args, g, df, node_feats, edge_feats):
    ...
    # user_torch_total_trainable_params = sum(
    # p.numel() for p in model.parameters() if p.requires_grad)
    # print(f'Total training time: {total_time}')
    # print(f'# of epoch {user_epoch_num}')
    # print(f'Time of epoch: {user_train_total_time / user_epoch_num}')
    # print(f'# of parms: {user_torch_total_trainable_params}')
